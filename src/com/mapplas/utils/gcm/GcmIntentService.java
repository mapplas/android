...
public class GcmIntentService extends IntentService {
	...
	private void showNotification(Bundle bundle) {
		...
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(this).setSmallIcon(R.drawable.ic_notification).setContentTitle(title).setStyle(new NotificationCompat.BigTextStyle().bigText(message)).setContentText(message).setAutoCancel(true).setTicker(textForNotificationBar).setSound(null).setWhen(System.currentTimeMillis());

		if(bundle.containsKey("type")) {
			int type = Integer.parseInt(bundle.getString("type"));
			switch (type) {
				case 1:
					Intent notificationIntentUrl = new Intent(Intent.ACTION_VIEW, Uri.parse(message));
					notificationIntentUrl.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
					PendingIntent contentIntent = PendingIntent.getActivity(this, 0, notificationIntentUrl, 0);
